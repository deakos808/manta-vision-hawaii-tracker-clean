...
// Phase 0 stub: use `any` so inner code like `db.queryArray` type-checks, but still throws at runtime.
async function withTx<T>(_fn: (db: any) => Promise<T>): Promise<T> {
  throw new Error(
...
// ---- MERGE (transactional via RPC) ----
if (action === "merge") {
  const p1 = parseIntId(payload?.primary_pk_catalog_id, "primary_pk_catalog_id");
  const p2 = parseIntId(payload?.secondary_pk_catalog_id, "secondary_pk_catalog_id");
  const del = !!payload?.delete_secondary_if_detached;

  if (!p1 || !p2 || p1 === p2) {
    return badRequest("Invalid catalog IDs");
  }

  const primary   = Math.min(p1, p2);
  const secondary = Math.max(p1, p2);

  try {
    const { data, error } = await admin.rpc("merge_catalogs_tx", {
      p_primary: primary,
      p_secondary: secondary,
      p_delete_secondary_if_detached: del,
    });
    if (error) {
...
// ---- end MERGE ----
  ...
  // ---- BEGIN TEMP HEALTH PROBE (remove after diagnostics) ----
if (action === "health") {
  try {
    // light DB check: can the admin client read one row?
...
// ---- END TEMP HEALTH PROBE ----

  if (action === "set_best_catalog_ventral") {
   return json(501, { ok: false, error: "set_best_catalog_ventral not implemented in Edge (will move to RPC)" });
