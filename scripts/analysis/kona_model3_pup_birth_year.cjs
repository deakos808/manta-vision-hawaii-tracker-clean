...
function normAgeClass(value) {
  ...
  if (v.includes("juven") || v.includes("immature")) return "immature";
  return "unknown";
}
...
function rankByScore(rows) {
  ...
  sorted.forEach((row, index) => {
    if (row.model3_score !== previousScore) currentRank = index + 1;
    row.model3_rank = currentRank;
    previousScore = row.model3_score;
  });
  return sorted;
}
...
function makeWorkbook(evidenceRows, rankRows, narrativeRows, summaryRows) {
  ...
  xlsx.utils.book_append_sheet(wb, ranks, "Model 3 Ranks");
...
async function main() {
  ...
  const rows = ranked.map((row) => {
    ...
    const lastEvidenceDate = dateOnly(row.effective_first_sighting && row.total_years_sighted ? addYears(row.effective_first_sighting, Math.max(0, Number(row.total_years_sighted) - 1)) : catalog.date_last_sighted);
    ...
    if (sourceGroup === "HAMER" && ageClassStatus === "mature") {
    ...
    } else if (sourceGroup === "HAMER" && (ageClassStatus === "immature" || ageClassStatus === "pup")) {
      immatureDate = biopsyDate;
      evidenceBasisParts.push(`HAMER biopsy-linked age class ${ageClassStatus}`);
      ...
      } else if (ageClassStatus === "immature" || ageClassStatus === "pup") {
        immatureDate = mprfFirst || biopsyDate;
        evidenceBasisParts.push(`MPRF provided age class ${ageClassStatus}`);
        const derivedMature = addYears(immatureDate, 15);
        if (derivedMature && lastEvidenceDate && compareDate(derivedMature, lastEvidenceDate) <= 0) {
          matureDate = derivedMature;
          evidenceBasisParts.push(`${ageClassStatus} / immature anchor + 15 years mature date`);
        }
      }
    ...
    const immaturePlusFifteenDate = immatureDate ? addYears(immatureDate, 15) : "";
    if (immaturePlusFifteenDate && lastEvidenceDate && compareDate(immaturePlusFifteenDate, lastEvidenceDate) <= 0) {
      if (!matureDate || compareDate(immaturePlusFifteenDate, matureDate) < 0) matureDate = immaturePlusFifteenDate;
      if (!evidenceBasisParts.some((part) => part.includes("immature anchor + 15 years mature date"))) {
        evidenceBasisParts.push(`${ageClassStatus} / immature anchor + 15 years mature date`);
      }
    }

    const model3Evidence = matureDate && immatureDate
      ? "mature_and_immature"
      : matureDate
        ? "mature"
        : immatureDate
    ...
    return {
      ...
      birth_year_anchor_date: ageClassStatus === "pup" ? immatureDate : "",
      immature_plus_15_mature_date: immaturePlusFifteenDate,
      ...
      model3_evidence: model3Evidence,
      model3_evidence_basis: evidenceBasis,
  ...
  const rankBase = rows.map((row) => {
    ...
    return {
      model3_rank: "",
      model3_score: olderThan - youngerThan,
      ...
      model3_evidence: row.model3_evidence,
      ...
      rank_note: olderThan === 0 && youngerThan === 0 ? "No Model 3 pairwise ordering evidence; tied by unresolved score." : "",
    };
  });
  ...
  const evidenceRows = [...rows].sort((a, b) => {
    ...
    birth_year_anchor_date: row.birth_year_anchor_date,
    immature_plus_15_mature_date: row.immature_plus_15_mature_date,
    ...
    model3_evidence: row.model3_evidence,
    model3_evidence_basis: row.model3_evidence_basis,
    current_view_rank: row.current_view_rank,
    review_note: row.source_group === "MPRF" ? "MPRF age class used in Model 3; maturity criteria behind that class remain uncertain." : "",
  }));

  const summaryCounts = {
    total: rows.length,
    hamer_total: rows.filter((r) => r.source_group === "HAMER").length,
    hamer_age_class_designated: rows.filter((r) => r.source_group === "HAMER" && ["mature", "immature", "pup"].includes(r.age_class_status)).length,
    ...
    mprf_age_class_designated: rows.filter((r) => r.source_group === "MPRF" && ["mature", "immature", "pup"].includes(r.age_class_status)).length,
    mprf_age_class_unknown: rows.filter((r) => r.source_group === "MPRF" && r.age_class_status === "unknown").length,
    pup_birth_year_anchors: rows.filter((r) => r.age_class_status === "pup").length,
    immature_plus_15_supported_mature: rows.filter((r) => r.immature_date && r.mature_date && r.model3_evidence === "mature_and_immature").length,
    mprf_over_15_years: rows.filter((r) => r.source_group === "MPRF" && Number(r.mprf_total_years_seen) > 15).length,
    mature_evidence: rows.filter((r) => ["mature", "mature_and_immature"].includes(r.model3_evidence)).length,
    immature_evidence: rows.filter((r) => ["immature", "mature_and_immature"].includes(r.model3_evidence)).length,
    no_model3_evidence: rows.filter((r) => !["mature", "immature", "mature_and_immature"].includes(r.model3_evidence)).length,
    pairwise_edges: edges.length,
  };

  const narrativeRows = [
    ["Model 3 purpose", "Rank the 83 Kona biopsy samples after treating date seen as pup as a birth-year anchor and deriving mature evidence 15 years after pup/immature anchors when sighting history supports it."],
    ["Why ranks repeat", "The model does not force an arbitrary total order. If two samples have the same Model 3 score, or no direct evidence separating them, they share a rank. The ranking spreadsheet has exactly 83 rows: one per biopsy sample."],
    ["HAMER rule", "For HAMER biopsy samples, the biopsy-linked manta age class is treated as high-confidence field evidence. Adult/mature = mature on the biopsy-linked sighting date. Juvenile/pup/immature = immature on that date. Unknown or blank = no Model 3 age evidence."],
    ["MPRF rule", "For MPRF biopsy samples, provided age class is used. Adult/mature = mature on the MPRF first/effective sighting date. Juvenile/immature = immature on the MPRF first/effective sighting date. Pup is treated as a birth-year anchor. Pup or immature anchors can also generate mature evidence at anchor date + 15 years when later sighting history supports that date."],
    ...
    ["Scientific caution", "Model 3 is less conservative than Models 1 and 2 because pup labels are interpreted as birth-year anchors. Treat changes as provisional until pup labels and sighting histories are reviewed."],
  ...
  const summaryRows = [
    ...
    { metric: "HAMER age class designated", value: summaryCounts.hamer_age_class_designated, note: "Mature, immature, or pup" },
    ...
    { metric: "MPRF age class designated", value: summaryCounts.mprf_age_class_designated, note: "Mature, immature, or pup in provided MPRF age class" },
    { metric: "MPRF age class unknown", value: summaryCounts.mprf_age_class_unknown, note: "Blank/unknown/not interpretable" },
    { metric: "Pup birth-year anchors", value: summaryCounts.pup_birth_year_anchors, note: "Pup date treated as birth year in Model 3" },
    { metric: "Immature/pup +15 mature dates supported", value: summaryCounts.immature_plus_15_supported_mature, note: "Derived mature date retained when later history supports it" },
    ...
    { metric: "Samples unresolved by Model 3", value: summaryCounts.no_model3_evidence, note: "No mature/immature Model 3 evidence" },
  ...
  const evidenceColumns = [
    ...
    "biopsy_source", "biopsy_date", "age_class_raw", "age_class_status", "birth_year_anchor_date", "immature_plus_15_mature_date", "last_evidence_date",
    "mprf_total_years_seen", "mprf_first_sighting_date", "mprf_15_year_mature_date",
    "model3_evidence", "model3_evidence_basis", "current_view_rank", "review_note",
  ];
  const rankColumns = [
    "model3_rank", "model3_score", "catalog_id", "name", "biopsy_id", "sample_id", "sample_lookup_name",
    "source_group", "model3_evidence", "mature_date", "immature_date",
    "older_than_count", "younger_than_count", "current_view_rank", "rank_note",
  ];

  const evidenceCsv = path.join(OUT_DIR, "kona_model3_evidence_by_source.csv");
  const ranksCsv = path.join(OUT_DIR, "kona_model3_ranks_83_samples.csv");
  const workbookPath = path.join(OUT_DIR, "kona_model3_pup_birth_year_age_rank.xlsx");
