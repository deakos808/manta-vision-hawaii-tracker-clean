...
export default function MapDialog({ open, onOpenChange, points }: Props) {
  ...
  // Decide engine whenever dialog opens
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    (async () => {
  ...
  // Defer init slightly after open for dialog layout
  useEffect(() => {
    if (open) {
      setReady(false);
  ...
  // Init fresh map whenever open+ready and engine are set
  useEffect(() => {
        ...
        const render = () => {
          ...
          clusters.forEach((c: any) => {
            ...
            if (c.properties.cluster) {
              ...
              const html =
                '<div style="background:#22c55e;border:1px solid #fff;width:10px;height:10px;border-radius:5px;' +
                'box-shadow:0 1px 2px rgba(0,0,0,0.25);"></div>';
              const icon = L.divIcon({ className: "cluster-pin", html, iconSize: [10,10], iconAnchor: [5,5] });
