...
export default function MapDialog({ open, onOpenChange, points, totalFiltered, children }: Props) {
  ...
  crossOrigin: true
            ...
            if (c.properties.cluster) {
              ...
              m.on("click", (evt: any) => {
                if (evt?.originalEvent?.altKey) {
                  // alt-click: list up to 10 sighting IDs in this cluster
        ...
        function setLeafletData(pts: Point[]) {
          ...
          const { L: LL } = mapRef.current;
          if (pts && pts.length) {
            const bounds = (LL as any).latLngBounds(pts.map(p => (LL as any).latLng(p.lat, p.lon)));
            if (bounds.isValid()) {
