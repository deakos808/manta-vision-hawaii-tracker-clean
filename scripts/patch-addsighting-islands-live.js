...
// 1) Replace hard-coded ISLANDS with live state + effect (using existing `supabase` import)
src = src.replace(
  /const\s+ISLANDS\s*=\s*\[[\s\S]*?\];\s*\n/,
`// Islands list now comes live from DB (public.sightings.island)
...
// 2) Replace the island <select> options to use the live list with render-time logging
src = src.replace(
...
// 3) Add a tiny on-screen probe under the Location block (just before "Notes")
...
if (src !== orig) {
  ...
  console.log("No changes applied (pattern not found or already patched).");
}
