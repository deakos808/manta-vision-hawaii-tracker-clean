...
// 1) Ensure handleSelectFromMap exists at component scope (before first "return (")
if (!/function\s+handleSelectFromMap\s*\(/.test(s) && !/const\s+handleSelectFromMap\s*=/.test(s)) {
...
`;
    ...
    console.log('added handleSelectFromMap');
  }
}

// 2) Pass onSelect={handleSelectFromMap} to MapDialog (once)
s = s.replace(/<MapDialog\b([^>]*?)\/>/, (m, attrs) => (/onSelect=/.test(attrs) ? m : `<MapDialog${attrs} onSelect={handleSelectFromMap} />`));
s = s.replace(/<MapDialog\b([^>]*?)>/, (m, attrs) => (/onSelect=/.test(attrs) ? m : `<MapDialog${attrs} onSelect={handleSelectFromMap}>`));

// 3) Ensure each Card has a single data-sighting-id attribute
...
console.log('Sightings.tsx hot-fixed');
