...
| Role/status changes | Direct browser updates and profile-only deletion. | Self-lockout, last-admin loss, deletion mismatch, weak audit. | Transactional RPC, reason and confirmation, self-protection, two-active-admin floor, audit row in same transaction; suspend/reactivate replaces deletion. | Active admins and `profiles`. | Paired rollback SQL; restore reviewed prior policies/grants separately. | Pure policy tests and SQL contract tests; database tests remain pending. |
| Invitations | Could create/recover accounts and expose setup links. | Link leakage, ambiguous operation, role/profile partial state. | Invite-only operation, always regular user, Auth-triggered default profile in the same Auth transaction, email delivery only, descriptive display name. | New invited users, Auth, `profiles`. | Drop trigger/function/audit objects with paired rollback. | Static security tests; synthetic local Supabase test pending. |
...
- The proposal intentionally aborts if `profiles` already has policies. Repository migrations do not contain a trustworthy prior-policy baseline, so silently adding policies could leave permissive policies effective.
- Before approval, reconcile the proposal against a fresh schema-only policy/grant inventory, adapt the migration to explicitly replace reviewed policies, and make the rollback restore their exact prior definitions.
...
## Decisions required from Mark

- Approve the exact production and local origins/redirect URLs (values belong in environment configuration, never Git).
- Decide whether the two-active-admin floor is permanent policy or an emergency override requires a separate break-glass process.
- Decide how the existing unmatched Auth account should eventually be reconciled; this checkpoint makes no change.
- Approve a reviewed schema-only reconciliation and local synthetic database test before any migration or function deployment.
