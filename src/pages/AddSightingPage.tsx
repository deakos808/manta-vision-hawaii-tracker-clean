...
export default function AddSightingPage() {
  ...
  // REVIEW FLAGS HOOK (idempotent)
  const [searchParams] = useSearchParams();



  // Non-destructive "Save Changes" for review drafts
  ...
  const saveReviewDraft = React.useCallback(async () => {
    if (!reviewIdParam) { alert("No review id"); return; }
    try {
      ...
      localStorage.setItem('__reviewDraft:' + reviewIdParam, JSON.stringify(payload));
      alert('Draft saved');
    } catch (e) {
  ...
  React.useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem('__reviewDraft:' + reviewIdParam);
      if (!raw) return;
      ...
      console.log('[AddSighting][review] hydrated draft for', reviewIdParam);
    } catch (e) {
  ...
  // ---- local draft save (temporary; server wiring comes next) ----
        // Collect notes from the page and mantas from global state (summary components populate this)
      const ta = document.querySelector('textarea[placeholder*="notes" i]') as HTMLTextAreaElement | null;
      const notes = ta?.value ?? '';
      const mantas = (window as any).__mantas ?? [];
      const payload = { notes, mantas, editedAt: new Date().toISOString() };

      const { error } = await supabase.from('sighting_submissions').update({ payload }).eq('id', rid);
      if (error) { console.warn('[review save] supabase error:', error.message); alert('Save failed'); return; }
      alert('Draft saved');
    } catch (e:any) {
      console.warn('[review save] exception:', e?.message || e);
      alert('Save errored');
    }
  }
...
useEffect(() => {
  ...
  // Review draft: non-destructive merge save
  const saveReviewServer = async () => {
    try {
      const rid = (typeof reviewId !== 'undefined' && reviewId) ? reviewId
                 : new URLSearchParams(window.location.search).get('review');
      if (!rid) { alert("No review id in URL"); return; }
      ...
      const { data: cur, error: fetchErr } = await supabase
        ...
        .eq('id', rid)
        .limit(1);
      if (fetchErr) { console.warn('[review save] fetch error:', fetchErr.message); alert('Save failed (fetch)'); return; }
      const prior = Array.isArray(cur) && cur[0]?.payload ? cur[0].payload : {};

      // Collect current UI state safely
      const getVal = (sel) => { const el = document.querySelector(sel); return (el && 'value' in el) ? String(el.value) : undefined; };
      const notes  = getVal('textarea[placeholder*="notes" i]') ?? prior.notes ?? '';

      const island = getVal('select[name="island"], select[data-field="island"]') ?? prior?.location?.island;
      const site   = getVal('select[name="sitelocation"], select[data-field="sitelocation"]') ?? prior?.location?.sitelocation;
      const latS   = getVal('input[name="latitude"], input[placeholder*="Latitude" i], input[data-field="lat"]');
      const lngS   = getVal('input[name="longitude"], input[placeholder*="Longitude" i], input[data-field="lng"]');
      const latitude  = latS !== undefined ? Number(latS) : prior?.location?.latitude;
      const longitude = lngS !== undefined ? Number(lngS) : prior?.location?.longitude;

      const mantas = (window as any).__mantas ?? prior.mantas ?? [];

      const payload = {
        ...prior,
        notes,
        location: { ...(prior.location ?? {}), island, sitelocation: site, latitude, longitude },
        mantas,
      ...
      const { error } = await supabase
        ...
        .update({ payload })
        .eq('id', rid);
      if (error) { console.warn('[review save] update error:', error.message); alert('Save failed'); return; }
...
const [islandsLoading, setIslandsLoading] = useState<boolean>(true);
  ...
  const formSightingId = useMemo(()=>genId(),[]);
  ...
  const onAddSave = (m: MantaDraft) => {
    ...
    setMantas(prev => {
      ...
      const id = exists || !incomingId ? genId() : incomingId;
...
<Button variant="destructive" onClick={handleRejectReview}>Reject</Button>
              ...
              <button type="button" data-clean-id="review-inline-save" className="ml-2 px-3 py-2 rounded border" onClick={saveReviewServer}>Save Changes</button>
...
// ---- merge + UI readers (idempotent) ----
function deepMerge(target: any, ...sources: any[]): any {
  const isObj = (o: any) => o && typeof o === 'object' && !Array.isArray(o);
  const out: any = Array.isArray(target) ? [...target] : isObj(target) ? { ...target } : {};
  for (const src of sources) {
    if (src == null) continue;
    if (Array.isArray(src)) return src.slice(); // replace arrays entirely
    for (const [k, v] of Object.entries(src)) {
      if (Array.isArray(v)) out[k] = v.slice();
      else if (isObj(v)) out[k] = deepMerge(out[k] ?? {}, v as any);
...
function readValue(selectors: string[]): string | undefined {
  ...
  for (const sel of selectors) {
    ...
    if (el && 'value' in el) {
      ...
      if (val !== undefined && String(val).length) return String(val);
    }
  }
  return undefined;
}
...
function getUiState() {
  ...
  const siteLocation = readValue(['[name="site"]', '[name="sitelocation"]', '#site', '#sitelocation', '[data-field="site"]', '[data-field="sitelocation"]']);
  ...
  if (typeof window !== 'undefined' && Array.isArray((window as any).__mantas)) {
    mantas = (window as any).__mantas;
  } else {
  ...
  const location = deepMerge({}, 
    island ? { island } : {},
    siteLocation ? { sitelocation: siteLocation, site: siteLocation } : {},
    lat !== undefined ? { lat } : {},
    lng !== undefined ? { lng } : {},
  );
  const out: any = {};
...
export async function saveReviewServer(reviewId: string, partial?: any) {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw error;
  const existing = (data && (data as any).payload) || {};
  const ui = getUiState();
  const merged = deepMerge({}, existing, ui, partial ?? {});
