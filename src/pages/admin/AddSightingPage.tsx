...
<div className="border rounded p-3 text-sm text-muted-foreground">
  Map preview temporarily disabled (WebGL not available). You can still enter coordinates manually above.
        ...
        <Card className="p-4 space-y-4">
          ...
          <Input type="number" value={totalMantasSeen} onChange={(e) => setTotalMantasSeen(e.target.value === '' ? '' : Number(e.target.value))} />
        </Card>
        ...
        <Button className="mt-6">Submit Sighting</Button>
      </div>
    </Layout>
  );
}
