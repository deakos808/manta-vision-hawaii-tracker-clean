...
export type MantaDraft = {
  ...
  size?: string | null; // keep as string in modal; page formats to 2dp when displaying
  photos: Uploaded[];
};
...
export default function UnifiedMantaModal({ open, onClose, sightingId, onSave, existingManta }: Props) {
  ...
  async function handleFiles(files: File[]) {
    ...
    for (const f of files) {
      if (!allow.includes(f.type)) {
        console.warn("[UnifiedMantaModal] skip type", f.type, f.name);
        continue;
      }
  ...
  return (
    <div
      className="fixed inset-0 bg-black/40 z-[300000] flex items-center justify-center"
      ...
      <div className="bg-white rounded-lg border w-full max-w-4xl p-4 pointer-events-auto relative">
        ...
        <div className="flex items-center justify-between mb-3 pr-10">
          <h3 className="text-lg font-medium">
            {existingManta ? "Edit Manta" : "Add Manta"}
          </h3>
          <div className="text-[11px] text-gray-500">
            sighting: {sightingId.slice(0, 8)}
          </div>
        </div>
                ...
                {photos.map((p) => (
                  <div key={p.id} className="border rounded p-2">
                    ...
                    <select
                      ...
                      value={p.view}
                      onChange={(e) => setView(p.id, e.target.value as View)}
                    >
