...
export default function MeasureModal({ open, src, onClose, onApply, initial }: Props) {
  ...
  const scalePx = useMemo(
    () => points.length >= 2 ? Math.hypot(px(points[0]).x - px(points[1]).x, px(points[0]).y - px(points[1]).y) : 0,
    [points, dims]
  );
  const discPx = useMemo(
    () => points.length >= 4 ? Math.hypot(px(points[2]).x - px(points[3]).x, px(points[2]).y - px(points[3]).y) : 0,
    [points, dims]
  );
    ...
    <div className="fixed inset-0 z-[500000] bg-black/60 flex items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded shadow-lg w-[min(1200px,95vw)] max-h-[90vh] overflow-hidden" onClick={(e)=>e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <div className="font-medium">Measure (Scale/Object with loupe). Drag points to adjust.</div>
        ...
        <div className="px-4 pt-3 pb-2 overflow-auto" style={{ maxHeight: "70vh" }}>
          <div className="inline-block relative">
            <img ref={imgRef} src={src} alt="measure" className="block max-h-[65vh] w-auto rounded" draggable={false} />
