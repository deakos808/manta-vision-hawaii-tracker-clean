...
export default function MantasList({ mantas, onEdit, onRemove, openMatch }: Props){
  ...
  const TD = "text-[13px] text-slate-700 whitespace-nowrap overflow-hidden text-ellipsis";
          ...
          return (
            <div key={String(m?.id ?? idx)} className={`${GRID} px-6 py-3 overflow-hidden pr-3`}>
              ...
              <div className="h-[136px] flex flex-col items-center justify-end">
                ...
                <div className="mt-2 flex items-center gap-2 max-w-[128px]">
                  {matchedId !== null ? (
                    <>
                      <span className="text-[11px] px-2 py-[3px] rounded-full bg-emerald-100 text-emerald-700">Matched</span>
                      <span className="text-[10px] text-slate-500 truncate">pk_catalog_id: {matchedId}</span>
                    </>
                  ) : noMatch ? (
                    <span className="text-[11px] px-2 py-[3px] rounded-full bg-slate-100 text-slate-600">No matches found</span>
                  ) : (
                    <button
                      type="button"
              ...
              <div className="h-[136px] flex items-end justify-center">
