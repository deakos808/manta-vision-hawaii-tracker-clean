...
export default function DronePhotosMapModal({ open, onOpenChange, title = "Photo Map", points }: Props) {
  ...
  function teardown() {
    try { overlayRef.current && (overlayRef.current as any).clearLayers && (overlayRef.current as any).clearLayers(); } catch (e) {}
    try { overlayRef.current && (overlayRef.current as any).remove && (overlayRef.current as any).remove(); } catch (e) {}
    overlayRef.current = null;
    try { mapRef.current && mapRef.current.remove && mapRef.current.remove(); } catch (e) {}
    mapRef.current = null;
    try {
