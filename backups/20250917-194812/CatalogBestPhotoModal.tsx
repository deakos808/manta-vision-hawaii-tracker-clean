...
type Props = {
  ...
  view?: View;
  onSaved?: (newThumb?: string | null) => void;
};
  ...
  async function onSave() {
    ...
    try {
      ...
      if (view === "ventral") {
        ...
        if (updateCatalog.error) throw updateCatalog.error;
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            Select Best {view === "ventral" ? "Ventral" : "Dorsal"} Photo
        ...
        {loading ? (
          ...
          <div className="p-2 text-sm text-muted-foreground">
            No {view} photos found for this catalog.
          </div>
        ) : (
          <RadioGroup
              ...
              return (
                <label key={p.pk_photo_id} className="block cursor-pointer">
                  <img
                    ...
                    onClick={(ev) => window.open(resolveImageUrl(p), "_blank")}
        ...
        <DialogFooter>
          ...
          <Button onClick={onSave} disabled={!pk_catalog_id || !selectedPhotoId || saving || loading}>
